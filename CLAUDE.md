...
| `cd gui && node scripts/check-log-filters.mjs` | Харнесс чистых функций фильтров лога |
...
- **Воронка мутаций.** Любая команда, меняющая репозиторий, идёт через `run(promise, label)`
  ...
  ревалидирует выделение. Компонент своего `try/catch` не пишет. Если нужен текст ошибки на
...
- `fileTree.ts` — **третья** реализация раскладки путей в проекте (первые две — в
  `ChangesView.tsx` и в дереве веток). Сведение в один модуль — записанный долг; четвёртую
  не заводить.
